...
def correlation_to_color(val):
    ...
    if abs_v <= 0.19:
        ...
        return "#9900FF"  # фиолетовый
    elif abs_v <= 0.79:
        return "#0066FF"  # синий
    else:
        return "#00CC33"  # зеленый
...
text="Метод:").pack(side=tk.LEFT)
    ...
    def show_stats(self):
        ...
        lbl = tk.Label(series_frame, text="Статистические ряды:")
...
title=f"t-критерий (t_критическое = {t_critical:.3f})",
    ...
    def display_df(self, df, title="", colorize=False, t_criteria_mode=False, t_critical=None, remove_pleiad_col=False, highlight_h0_accept=False):
        ...
        if highlight_h0_accept and t_critical is not None:
            try:
                # ищем столбец с t-статистиками (например, 't-стат.' или 't' и т.п.)
                t_col_candidates = [c for c in df.columns if 't' in str(c).lower()]
                if t_col_candidates:
                    t_col = t_col_candidates[0]
                    t_col_idx = df.columns.get_loc(t_col)
                    for i, row in enumerate(data):
                        val = row[t_col_idx + 1]  # +1 из-за row_labels
                        try:
                            t_val = abs(float(str(val).replace(',', '.')))
                            if t_val <= t_critical:
                                # выделить всю строку зеленым
                                self.sheet_widget.highlight_rows(rows=[i], bg="#C6FFCC", fg="black", overwrite=True)
                        except:
                            continue
            except Exception as e:
        ...
        if colorize and not t_criteria_mode:
            for i, row in enumerate(data):
                for j, val in enumerate(row):
                    ...
                    if color:
                        self.sheet_widget.highlight_cells(row=i, column=j, fg=color, overwrite=True)
